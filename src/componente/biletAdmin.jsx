...
const[esteActiv5,setEsteActiv5] = useState(false);
  ...
  useEffect(()=>{
      if(state!=null)
      {
        const { bilet } = state;
        setTip(bilet.tip);
        setNumeBilet(bilet.numeBilet);
        setValabilitateInfo(bilet.valabilitateInfo);
        setValabilitateTip(bilet.valabilitateTip);
        setNominal(bilet.nominal);
        setPerioada(bilet.perioada);
        setActiv(bilet.activ);
        setTipPersoana(bilet.tipPersoana);
        setInVanzare(bilet.inVanzare);
        setPret(bilet.pret);
        setId(bilet._id);
      }
    },[state]);
    ...
    },[valabilitateTip,perioada,tipPersoana,activ,nominal,tip]);
    ...
    const handleModificare = async (e)=>{
        ...
        try {
            ...
            await axiosJWT.post('http://localhost:3001/private/bileteActualizeaza/'+id,{tip:tip,numeBilet:numeBilet,pret:pret,valabilitateInfo:valabilitateInfo,valabilitateTip:valabilitateTip,nominal:nominal,perioada:perioada,activ:activ,tipPersoana:tipPersoana,inVanzare:inVanzare},config);
          ...
          <div >
            <form onSubmit={handleModificare}>
                ...
                {tip==="abonament"&&(<div>
                    ...
                    <div onClick={()=>{setEsteActiv1(!esteActiv1)}}>Alege una: {nominal} {activ.toString()}</div>
                    {esteActiv1 && (<div>
                ...
                <div>
                    ...
                    <div onClick={()=>{setEsteActiv3(!esteActiv3)}}>Alege una: {inVanzare.toString()}</div>
                    {esteActiv3 && (<div>
